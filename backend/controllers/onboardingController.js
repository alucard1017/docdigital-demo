...
exports.getOnboardingStatus = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            // Crear onboarding si no existe
            const newOnboarding = await pool.query(
...
exports.updateOnboardingStep = async (req, res) => {
    try {
            ...
            `UPDATE user_onboarding 
             SET current_step = $1,
                 steps_completed = CASE 
                     WHEN $2 IS NOT NULL AND NOT (steps_completed @> to_jsonb($2::text))
        ...
        res.status(500).json({ error: 'Error al actualizar paso del onboarding' });
    }
};
...
exports.skipOnboarding = async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `UPDATE user_onboarding 
             SET skipped = TRUE, completed = TRUE, completed_at = CURRENT_TIMESTAMP
             WHERE user_id = $1 AND company_id = $2
             RETURNING *`,
            [userId, companyId]
        );
        ...
        res.status(500).json({ error: 'Error al saltar onboarding' });
    }
};
...
exports.getTourProgress = async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT * FROM product_tour_progress 
        ...
        res.status(500).json({ error: 'Error al obtener progreso del tour' });
    }
};
...
exports.updateTourProgress = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al actualizar progreso del tour' });
    }
};
