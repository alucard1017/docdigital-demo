...
export function Timeline({ timeline }) {
  if (!timeline || !timeline.events) {
    return <div style={{ padding: '20px', textAlign: 'center', color: '#94a3b8' }}>Cargando progreso...</div>;
  }
  ...
  return (
    <div className="timeline-container">
      ...
      <div className="timeline-header">
        <h3 style={{ margin: '0 0 16px 0', color: '#1e293b', fontSize: '1.2rem', fontWeight: 800 }}>
        ...
        {/* Estado actual */}
        <div style={{ marginTop: '16px', padding: '12px 16px', background: '#f0f9ff', borderRadius: '12px', border: '1px solid #bae6fd' }}>
          <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '4px' }}>Estado Actual</div>
          <div style={{ fontSize: '1rem', fontWeight: 700, color: '#0369a1' }}>{currentStep}</div>
          <div style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '6px' }}>Próximo: {nextStep}</div>
        </div>
      </div>
        ...
        {events.map((event, index) => {
          ...
          return (
            <div key={event.id} className="timeline-event-wrapper">
              ...
              {!isLast && <div className={`timeline-line timeline-line-${status}`}></div>}
              ...
              <div className={`timeline-content timeline-content-${status}`}>
                <h4 style={{ margin: '0 0 6px 0', fontSize: '0.95rem', fontWeight: 700, color: '#1e293b' }}>
                  ...
                  {event.action === 'FIRMADO_REPRESENTANTE' && '✓ Firmado por Representante'}
                </h4>
                ...
                {event.actor && (
                  <p style={{ margin: '4px 0 0 0', fontSize: '0.75rem', color: '#7c3aed', fontWeight: 600 }}>
                    Por: {event.actor}
                  </p>
                )}
              </div>
